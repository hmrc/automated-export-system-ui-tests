 ...
 *
 * Unlike SubmissionDiscrepanciesSpec (which combines MUCR with a split
 * exit and discrepancies), this covers MUCR association on its own, per
 * the TC17 test case definition.
 */
class SubmissionMucrSpec extends BaseSpec {

  Feature("IE507(a) Submission with MUCR association") {

    Scenario("E2E Journey: Submit a IE507(a) Declaration with a MUCR reference, no discrepancies") {
      ...
      And("the MUCR I entered is shown in the answers")
      CheckYourAnswersPage.checkForContent(mucr)
